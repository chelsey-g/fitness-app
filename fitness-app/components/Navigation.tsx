...
//                     <div className="absolute right-0 mt-2 w-48 bg-white text-gray-800 shadow-lg z-10 text-sm rounded transition duration-200 ease-in-out transform scale-95">
      ...
      {isMenuOpen && (
        <div className="lg:hidden mt-4 space-y-2">
          {isLoggedIn ? (
              ...
              {DROPDOWN_ITEMS.map((dropdown) => (
                <div key={dropdown.id}>
                  <button
                    ...
                    className="flex items-center justify-between w-full px-4 py-2 bg-gray-700 text-white"
                  >
                    <span>{dropdown.label}</span>
                    <IoIosArrowDown />
                  </button>
            ...
            <Link
              href="/login"
              className="block px-4 py-2 bg-logo-green text-black hover:opacity-90"
            >
              Login
            </Link>
          )}
